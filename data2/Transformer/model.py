...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):  # x: [B, T, D]
        x = x + self.pe[:x.size(1)].unsqueeze(0).to(x.device)  # [1, T, D]
        return x
...
class TransformerALL(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, dropout):
        ...
        self.fc_out = nn.Linear(d_model, 1)  # 每步一个回归值
    ...
    def forward(self, src, tgt):
        ...
        output = self.decoder(tgt, memory)  # [B, T, D]

        return self.fc_out(output).squeeze(-1)  # [B, T]
